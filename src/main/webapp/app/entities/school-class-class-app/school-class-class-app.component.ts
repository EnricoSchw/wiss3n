...
@Component({
    selector: 'jhi-school-class-class-app',
    templateUrl: './school-class-class-app.component.html'
})
export class SchoolClassClassAppComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.schoolClassService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<SchoolClassClassApp[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/school-class-class-app'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/school-class-class-app', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: SchoolClassClassApp) {
        return item.id;
    }
