...
describe('Tag e2e test', () => {
    ...
    it('should load Tags', () => {
        navBarPage.goToEntity('tag-class-app');
        tagComponentsPage = new TagComponentsPage();
        expect(tagComponentsPage.getTitle())
            .toMatch(/klassenchatappApp.tag.home.title/);
    ...
    it('should load create Tag dialog', () => {
        ...
        expect(tagDialogPage.getModalTitle())
            .toMatch(/klassenchatappApp.tag.home.createOrEditLabel/);
        tagDialogPage.close();
    });
...
export class TagComponentsPage {
    ...
    title = element.all(by.css('jhi-tag-class-app div h2 span')).first();
